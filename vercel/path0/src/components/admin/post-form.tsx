...
export function PostForm({ post }: { post?: SerializablePost }) {
  ...
  const form = useForm<PostFormData>({
    ...
    defaultValues: {
      title: post?.title || '',
      slug: post?.slug || '',
      content: post?.content || [{ type: 'paragraph', value: '' }],
            ...
            {fields.map((field, index) => (
              <div key={field.id} className="flex gap-2 items-start p-4 border rounded-lg bg-muted/50">
                ...
                <div className="flex-1 space-y-2">
                  {field.type === 'paragraph' ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        {...form.register(`content.${index}.value.url`)}
                      />
                      <Input
                        placeholder="Alt text for accessibility"
                        {...form.register(`content.${index}.value.alt`)}
                      />
                    </div>
                  )}
                  ...
                  {form.formState.errors.content?.[index]?.value?.url && <p className="text-sm text-destructive">{form.formState.errors.content?.[index]?.value?.url?.message}</p>}
                </div>
                <div className="pt-1">
