...
async function PostsData({ tag }: { tag?: string }) {
  let posts = await getPosts({ publishedOnly: true, tag: tag || 'World Security' });
...
async function Tags() {
    ...
    return <TagList tags={tags} activeTag="World Security" />;
}
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 md:px-6 py-12">
            <div className="text-center mb-12">
                <h1 className="text-4xl md:text-5xl font-bold font-headline text-primary">World Security</h1>
                <p className="text-lg text-muted-foreground mt-2">Insights on global security and defense</p>
            </div>
             <div className="mb-8">
                <Suspense>
                    <Tags />
